...
// QueryMemberStats chains the current query on the "member_stats" edge.
func (_q *SnapshotQuery) QueryMemberStats() *MemberStatQuery {
	query := (&MemberStatClient{config: _q.config}).Query()
...
// QueryMemberYearStats chains the current query on the "member_year_stats" edge.
func (_q *SnapshotQuery) QueryMemberYearStats() *MemberYearStatQuery {
	query := (&MemberYearStatClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryMemberRepoStats chains the current query on the "member_repo_stats" edge.
func (_q *SnapshotQuery) QueryMemberRepoStats() *MemberRepoStatQuery {
	query := (&MemberRepoStatClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithMemberStats tells the query-builder to eager-load the nodes that are connected to
// the "member_stats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SnapshotQuery) WithMemberStats(opts ...func(*MemberStatQuery)) *SnapshotQuery {
	query := (&MemberStatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMemberStats = query
	return _q
}

// WithMemberYearStats tells the query-builder to eager-load the nodes that are connected to
// the "member_year_stats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SnapshotQuery) WithMemberYearStats(opts ...func(*MemberYearStatQuery)) *SnapshotQuery {
	query := (&MemberYearStatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithMemberRepoStats tells the query-builder to eager-load the nodes that are connected to
// the "member_repo_stats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SnapshotQuery) WithMemberRepoStats(opts ...func(*MemberRepoStatQuery)) *SnapshotQuery {
	query := (&MemberRepoStatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(snapshot.FieldCapturedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Snapshot.Query().
//		Select(snapshot.FieldCapturedAt).
//		Scan(ctx, &v)
...
func (_q *SnapshotQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Snapshot, error) {
	...
	if query := _q.withMemberStats; query != nil {
		if err := _q.loadMemberStats(ctx, query, nodes,
			func(n *Snapshot) { n.Edges.MemberStats = []*MemberStat{} },
			func(n *Snapshot, e *MemberStat) { n.Edges.MemberStats = append(n.Edges.MemberStats, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMemberYearStats; query != nil {
	...
	if query := _q.withMemberRepoStats; query != nil {
...
func (_q *SnapshotQuery) loadMemberStats(ctx context.Context, query *MemberStatQuery, nodes []*Snapshot, init func(*Snapshot), assign func(*Snapshot, *MemberStat)) error {
	...
	query.withFKs = true
	query.Where(predicate.MemberStat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(snapshot.MemberStatsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.snapshot_member_stats
		if fk == nil {
			return fmt.Errorf(`foreign-key "snapshot_member_stats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "snapshot_member_stats" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SnapshotQuery) loadMemberYearStats(ctx context.Context, query *MemberYearStatQuery, nodes []*Snapshot, init func(*Snapshot), assign func(*Snapshot, *MemberYearStat)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Snapshot)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.MemberYearStat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(snapshot.MemberYearStatsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.snapshot_member_year_stats
		if fk == nil {
			return fmt.Errorf(`foreign-key "snapshot_member_year_stats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *SnapshotQuery) loadMemberRepoStats(ctx context.Context, query *MemberRepoStatQuery, nodes []*Snapshot, init func(*Snapshot), assign func(*Snapshot, *MemberRepoStat)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Snapshot)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.MemberRepoStat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(snapshot.MemberRepoStatsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.snapshot_member_repo_stats
		if fk == nil {
			return fmt.Errorf(`foreign-key "snapshot_member_repo_stats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
